...
BEGIN
  ...
  -- Check if this was the last account for this plaid_item
  -- If so, delete the plaid_item to properly update the linked connections count
  IF NOT EXISTS (
    SELECT 1 FROM plaid_accounts WHERE plaid_item_id = v_account_record.plaid_item_id
  ) THEN
    DELETE FROM plaid_items WHERE id = v_account_record.plaid_item_id;
  END IF;
