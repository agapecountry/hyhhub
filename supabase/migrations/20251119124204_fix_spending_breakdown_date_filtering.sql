/*
  # Fix Spending Breakdown Date Filtering

  1. Problem
    - The spending breakdown function was incorrectly comparing payment_day with day-of-month extracts
    - This caused debts/bills from any month to appear if their payment_day was within the numeric range
    - Example: A debt with payment_day=5 in December would show in November breakdown because 5 is between 1-30

  2. Solution
    - Construct actual dates using make_date() to check if payment falls within the date range
    - For debts: make_date(EXTRACT(YEAR FROM p_start_date), EXTRACT(MONTH FROM p_start_date), d.payment_day)
    - For bills: make_date(EXTRACT(YEAR FROM p_start_date), EXTRACT(MONTH FROM p_start_date), b.due_date)
    - Compare these constructed dates against p_start_date and p_end_date

  3. Impact
    - Scheduled debts and bills now only appear in their correct month
    - Fixes incorrect spending totals showing future/past scheduled payments
*/
...
BEGIN
  ...
  debt_spending AS (
    ...
    WHERE d.household_id = p_household_id
      ...
      -- Construct the actual payment date and check if it falls within the range
      AND make_date(
        EXTRACT(YEAR FROM p_start_date)::int,
        EXTRACT(MONTH FROM p_start_date)::int,
        LEAST(d.payment_day, EXTRACT(DAY FROM (DATE_TRUNC('MONTH', p_start_date) + INTERVAL '1 MONTH - 1 day'))::int)
  ...
  bill_spending AS (
    ...
    WHERE b.household_id = p_household_id
      ...
      -- Construct the actual due date and check if it falls within the range
      AND make_date(
        EXTRACT(YEAR FROM p_start_date)::int,
        EXTRACT(MONTH FROM p_start_date)::int,
        LEAST(b.due_date, EXTRACT(DAY FROM (DATE_TRUNC('MONTH', p_start_date) + INTERVAL '1 MONTH - 1 day'))::int)
      ) BETWEEN p_start_date AND p_end_date
