/*
  ...
  Updates the get_spending_breakdown function to exclude transfer transactions
  from the spending breakdown. Transfers are internal movements between accounts
  and should not count as spending.
  
  Changes:
  - Adds filter to exclude transactions where category type = 'transfer'
  - Ensures budget reports only show actual income/expense transactions
*/
...
BEGIN
  ...
  -- UPDATED: Exclude transfer transactions
  transaction_spending AS (
    ...
    WHERE t.household_id = p_household_id
      ...
      AND t.amount < 0
      AND (tc.type IS NULL OR tc.type != 'transfer')  -- Exclude transfers
  ),
...
-- Add comment explaining the exclusion
COMMENT ON FUNCTION get_spending_breakdown(uuid, date, date) IS 
'Calculates spending breakdown by category for a given date range. Excludes transfer transactions as they are internal movements between accounts, not actual spending.';
